...
如今开发者往往同时使用多种 AI 编程助手（如 Codex、Claude Code、Hermes、OMP、DSH 等），会话记录分散在各自的配置目录、SQLite 数据库或自定义压缩日志中，历史难以统一翻阅。AI Session Hub 能够在本地实时读取各工具的会话转录与用量事实，建立独立的 SQLite FTS5 高性能检索索引，提供多维度的用量与消费看板，并支持快捷键一键安全恢复至原生工具终端。
...
* **贡献日历热力图（Calendar Heatmap）**：7 行周历热力图，通过深浅色块直观展现过去一整年的每日活跃度。
* **双列模型与工具占比拆解**：
